...
func (h *Handler) CalculateLuck(target *data.Player, left *data.Player, right *data.Player) int {
	if !left.Alive && !right.Alive {
		return 0 + target.LuckModifier
	}

	luck := 0
	tRole, _ := h.models.Players.GetRole(target.RoleID)
	lRole, _ := h.models.Players.GetRole(left.RoleID)
	rRole, _ := h.models.Players.GetRole(right.RoleID)

	if !left.Alive || lRole.Name == "Vagabond" {
		luck += 0
	} else {
		luck += calculateNeighborLuck(tRole.Alignment, lRole.Alignment)
	}

	if !right.Alive || rRole.Name == "Vagabond" {
		luck += 0
	} else {
		luck += calculateNeighborLuck(tRole.Alignment, rRole.Alignment)
	}
	return luck + target.LuckModifier
}
