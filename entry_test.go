...
func TestReadRoleCSV(t *testing.T) {
	...
	filepath := "./Masque Royale 2.1 Information Spreadsheet [2024] - ROLES.csv"
...
func TestReadAnyAbilityCSV(t *testing.T) {
	dbStr := os.Getenv("DATABASE_URL")
	db, err := sqlx.Open("postgres", dbStr)
	if err != nil {
		t.Fatal(err)
	}
	filepath := "./Masque Royale 1.1 Information Spreadsheet [2024] - AAs.csv"
	entries, err := ReadAnyAbilityCSV(filepath)
	if err != nil {
		t.Fatal(err)
	}
	for _, v := range entries {
		...
		_, err := db.NamedExec("INSERT INTO any_abilities (name, description, rarity, categories) VALUES (:name, :description, :rarity, :categories)", aa)
		if err != nil {
			t.Fatal(err)
		}
	}
...
func TestReadActionCSV(t *testing.T) {
	dbStr := os.Getenv("DATABASE_URL")
	db, err := sqlx.Open("postgres", dbStr)
	if err != nil {
		t.Fatal(err)
	}
	filepath := "./Masque Royale 2.1 Information Spreadsheet [2024] - Sheet14.csv"
	entries, err := ReadAnyAbilityCSV(filepath)
	if err != nil {
		t.Fatal(err)
	}
	for _, v := range entries {
		_, err := db.NamedExec("INSERT INTO actions (ability_name, description, shorthand, rarity, role_association, categories) VALUES (:ability_name, :description, :shorthand, :rarity, :role_association, :categories)", v)
		if err != nil {
			t.Fatal(err)
		}
	}
