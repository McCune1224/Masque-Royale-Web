...
func ReadAnyAbilityCSV(filepath string) ([]data.AnyAbility, error) {
	file, err := os.Open(filepath)
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		aa.Name = strings.TrimSpace(record[2])
		aa.Description = strings.TrimSpace(record[4])
		aa.Rarity = strings.TrimSpace(record[1])
		categories := strings.Split(record[3], "/")
		for _, c := range categories {
			aa.Categories = append(aa.Categories, strings.TrimSpace(c))
		}
...
func ReadActionCSV(filepath string) ([]data.Action, error) {
	file, err := os.Open(filepath)
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		action.Rarity = record[1]
		action.AbilityName = record[2]
		action.Shorthand = record[3]
		action.RoleAssociation = record[4]
		categories := strings.Split(record[5], "/")
		for _, c := range categories {
			action.Categories = append(action.Categories, strings.TrimSpace(c))
		}
		action.Description = record[6]
